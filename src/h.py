...
class text(html_item):
    ...
    def __init__(self, item, **data):
        super().__init__(item=item, **data)

    def __str__(self):
        return escape(str(self.item))
...
class comment(html_item):
    ...
    def __init__(self, contents, **data):
        super().__init__(contents=contents, **data)

    def __str__(self):
        # HTML comments cannot contain "--" or end with "-"
        content = str(self.contents).replace("--", "- -")
        if content.endswith("-"):
            content += " "
        return f"<!--{content}-->"
...
class tag(html_item):
    ...
    def __getitem__(self, key):
        if self.children:
            ...
            raise ValueError(f"Void tag <{self.tag_name}> cannot have children")

        if isinstance(key, (Generator, Iterable)) and not isinstance(key, (str, html_item)):
            children = key
        else:
            children = [key]
        ...
        return self.model_copy(update={"children": tuple(processed_children)})
    ...
    def __repr__(self):
        ...
        if self.children:
            ...
            if attrs_str:
                return f"{self.tag_name}({attrs_str}){children_repr}"
            return f"{self.tag_name}(){children_repr}"
        if attrs_str:
            return f"{self.tag_name}({attrs_str})"
...
fencedframe = tag_class("fencedframe")  # Experimental
...
search = tag_class("search")
section = tag_class("section")
select = tag_class("select")
selectedcontent = tag_class("selectedcontent")  # Experimental Non-standard
slot = tag_class("slot")
